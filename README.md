...
| **Offline self-test** | Ingestion + retrieval logic is verifiable without an LLM. |
...
# verify the pipeline logic without an LLM
...
flowchart LR
    AG["MCP client"]
    AG -->|search_evidence| EV["Passages + ids"]
    AG -->|get_passage| PA["One passage"]
    AG -->|verify_citation| VC["Claim check"]
    AG -->|describe_index| ST["Index scope"]
    AG -->|answer_question| AN["Cited answer"]
    EV --- N1["no LLM"]
    PA --- N1
    VC --- N1
    ST --- N1
