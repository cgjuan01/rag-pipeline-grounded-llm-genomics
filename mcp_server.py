...
Design notes
------------
* `search_evidence`, `get_passage` and `describe_index` need no LLM -- no
  Ollama, no API key, no GPU. They do use the sentence-transformers embedding
  model, which downloads once (~90MB) and is cached thereafter, so the first
  run needs network access and later ones do not. The calling agent supplies
...
-----
    ...
    python mcp_server.py --selftest # exercise every tool, no LLM needed
...
def answer_question(question: str, k: int = 6, mix: bool = True) -> dict:
    ...
    it needs no LLM and leaves the reasoning with you. Use this when you
...
def _selftest() -> int:
    """Exercise every tool that does not require an LLM."""
    ...
    print("selftest OK - every tool exercised without an LLM")
    return 0
