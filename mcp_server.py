...
ground its own reasoning in this corpus instead of answering from memory.
...
Design notes
------------
* `search_evidence`, `get_passage` and `corpus_stats` need no LLM and no
...
-----------
    ...
    LLM_BACKEND  transformers | ollama | anthropic | openai  (answer_question only)
...
mcp = FastMCP(
    "ask-my-research",
    instructions=(
        "Grounded retrieval over a biomedical research corpus of papers and "
        ...
        "the corpus does not support an answer rather than filling the gap "
...
def search_evidence(query: str, k: int = 6, kind: str | None = None) -> dict:
    ...
    if not hits:
        return {"grounded": False, "reason": "No passages in the corpus.",
                "passages": []}
    ...
    if best < MIN_SCORE:
        return {
            ...
            "reason": (
                ...
                "floor. This corpus does not appear to cover the query. Say so "
...
def get_passage(chunk_id: str) -> dict:
    ...
    Args:
        chunk_id: An id from search_evidence, e.g. "paper:mr_gat#3" or
            "table:convergent#CTSF".
    """
...
def corpus_stats() -> dict:
    ...
    return {
        ...
        "scope": ("Ageing and exercise genomics: the author's own papers and "
                  "structured model outputs. Not a general biomedical corpus."),
    }
...
def answer_question(question: str, k: int = 6, mix: bool = True) -> dict:
    ...
    llm = get_backend(backend, os.environ.get("LLM_MODEL"))
    k = max(1, min(int(k), 20))
    return RAG(_get_index(), llm).answer(question, k=k, mix=mix)


def _selftest() -> int:
    """Exercise every no-model tool against the configured corpus."""
    print(f"corpus: PAPERS_DIR={PAPERS_DIR} TABLES_DIR={TABLES_DIR}")
    stats = corpus_stats()
